...
class StepExecutor {
  ...
  /// Builds the prompt for the report-generator step.
  ///
  /// This prompt carries only the PROCESS the model follows (read, gather,
  /// score, write, save). The FORMAT — section order, section count, the
  /// fields inside a scored section, the score-label bands, and the shape
  /// of the trailing blocks — belongs to the skill's own template and rule
  /// file, which this prompt points at instead of restating. This prompt
  /// used to hardcode those facts (ascending sort order, a fixed metadata
  /// block, a one-size field list); they contradicted the templates of
  /// most of the 16 skills that share this builder. Do not re-add them —
  /// add a pointer to the template/rule file instead.
  String _buildReportPrompt(
    ...
    return 'You are generating the final audit report for the '
        ...
        '- Compute each section score using the rubrics\n'
        '- Compute the weighted Overall Score using the formula\n'
        '- Determine each score\'s label using the bands defined in the '
        'rule file or template — do not assume a particular set of bands\n'
        'A report without computed scores is INVALID and must not '
        'be produced.\n\n'
        'STEP 4: Write the report\n'
        'Generate the complete report following the section sequence the '
        'template defines, and apply whatever ordering rule the template '
        'or rule file states for those sections — nothing more, nothing '
        'less (for example, some templates fix a specific section order; '
        'others require the scored sections to be sorted dynamically, '
        'such as by score ascending). The template and rule file are the '
        'sole source of the section order; follow them exactly and '
        'contribute no ordering choice of your own. Give every scored '
        'section exactly the fields its template section shows, no more '
        'and no fewer.\n\n'
        'STEP 5: Close the report the way the template does\n'
        'After the last scored section, add whatever trailing blocks the '
        'template defines, in the template\'s order and shape (for example, '
        'an Appendix only if and where the template has one, and a Report '
        'Metadata block only in the shape — table or otherwise — the '
        'template renders it in). Where the template has a Report Metadata '
        'block, populate it with: Generated by Somnio CLI '
        'v$packageVersion, Skill ${config.bundleName}, Date '
        '[today\'s date in YYYY-MM-DD format], and Somnio AI Tools '
  ...
  /// Builds the prompt for the format-enforcer pass over a generated report.
  ///
  /// Like [_buildReportPrompt], this carries only PROCESS. It used to assert
  /// a single hardcoded score-label band scale (security-audit's) applied
  /// to all 16 skills sharing this builder, most of which use a different
  /// scale (or a differently named Band instead of a Label). Do not re-add
  ...
  String _buildFormatEnforcerPrompt(String ruleFile, String reportPath) {
    ...
    return 'You are validating and enforcing formatting on the '
        ...
        '- Verify all scored sections have a "Score:" line in the shape '
        'the template shows (e.g. "[Score]/100 (...)") — the parenthesized '
        'part may be a Label, a Band, or another name; use whatever the '
        'template calls it\n'
        '- Verify each score\'s label/band matches the bands this skill\'s '
        'own template and rule file define — these differ per skill, so '
        'never assume a particular set of bands\n'
