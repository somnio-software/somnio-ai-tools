...
Automatically fill Clockify from your `~/.work-log/` daily files (generated by the [work-log Stop hook](work-log-stop-hook.md)).
...
3. Map repos to Clockify projects (saved in `~/.clockify-prefs.json` — asked once, reused forever)
4. Generate a 2-sentence executive summary per entry
5. Show a full preview of all entries before posting
6. Handle days with multiple projects by asking for the hour split

**Preferences** (`~/.clockify-prefs.json`): stores your API key, timezone, default time block, workspace, and repo→project mappings. All values are saved on first use and reused automatically.

**API key:** Resolved in order — `CLOCKIFY_API_KEY` env var → `~/.clockify-prefs.json` → prompted once and optionally saved.

**Output:** Confirmed time entry (or entries) created via Clockify API, with a preview table shown before posting.
