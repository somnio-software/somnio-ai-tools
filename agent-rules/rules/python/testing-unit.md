...
- Using `assert x == True` instead of `assert x` — defeats pytest's rewriting and produces weaker failure messages.
- Defining fixtures in test modules instead of `conftest.py` — fixtures defined outside `conftest.py` cannot be shared across files without explicit imports.
- Using `scope="session"` on fixtures that mutate state — causes test-order dependency; a test that passes alone may fail in CI due to leftover state.
- Calling `@pytest.mark.parametrize` without `ids` — node IDs like `test_add[1-2-3]` instead of `test_add[positive_integers]` make failure reports hard to read.
- Writing `with pytest.raises(ValueError):` without `match` — only confirms the type; a completely different error message goes undetected.
- Using `mocker.patch("requests.get")` when the SUT imports `from requests import get` — the mock patches the wrong binding and the real function is called.
- Creating mocks without `autospec=True` — allows calling the mock with the wrong arguments; the bug is invisible until production.
- Relying on Hypothesis for integration scenarios — `@given` is for pure-function invariants; stateful/DB tests should use pytest fixtures with controlled data, not property strategies.
- Over-asserting implementation details (`assert mock.call_count == 3`) instead of observable behaviour — brittle tests that break on valid refactors.

## Purpose

These rules enforce the pytest community's established good practices for unit tests: clear naming, composable fixtures, expressive assertions, and minimal, correctly scoped mocks. Together they produce a test suite that is readable, deterministic, and safe to refactor.
