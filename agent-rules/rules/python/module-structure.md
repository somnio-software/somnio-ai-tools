...
- Placing package code at the repo root (`mypackage/` next to `tests/`) instead of under `src/` — this means `import mypackage` resolves to the local directory rather than the installed package, masking missing `__init__.py` files and import errors.
- Exporting everything from `__init__.py` without an explicit `__all__`, making it impossible to distinguish the public API from internal helpers.
- Using relative imports (`from .. import foo`) — they break under `src/` layout if a file is run directly and make refactoring harder.
- Skipping `pyproject.toml` in favour of a bare `setup.py` or `setup.cfg`, which are legacy formats that do not support PEP 517 build isolation.
- Importing a concrete infrastructure class (e.g. `PostgresRepository`) directly inside a domain service — this couples business logic to a driver and makes unit-testing require a live database.
- Calling `structlog.configure()` or `logging.basicConfig()` inside a library module at module-level — the first import of the library silently changes the host app's logging.
- Having more than one place that assembles the object graph, so the same interface is wired to different concrete implementations in different contexts (duplicated/conflicting composition roots).
- Circular imports caused by placing shared types inline in a feature module that other modules also import — resolve by extracting shared types to a standalone `types.py` or `interfaces.py`.

## Purpose

Consistent module structure ensures:
- The installed package is always tested, not a shadow of local files (`src/` layout).
- Public APIs are explicit and stable (`__all__`), reducing accidental breakage.
- Business logic is reusable and framework-independent (layered architecture + DI).
- The dependency graph is acyclic and navigable (absolute imports, no circulars).
- Packaging is standards-compliant and build-tool agnostic (`pyproject.toml`).
- Logging is predictable across library boundaries (no import-time configuration).
