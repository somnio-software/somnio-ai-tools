...
- Writing functions that both compute a result **and** write it to a file/database — split into two functions.
- Arrow anti-pattern: three or more levels of nested `if`/`else` instead of early returns.
- Returning `None` implicitly when the caller expects a value — missing `return` in some branches. (Source: https://google.github.io/styleguide/pyguide.html.)
- Mutating a list or dict argument in-place without documenting it, causing unexpected caller-side changes.
- Using positional-only boolean flags (`do_thing(True, False)`) — the call site is unreadable; use keyword-only args.
- Using a plain `dict` as a function's return type when a `@dataclass` would make fields explicit and type-checkable.
- Defining `default_factory` as a mutable literal (e.g., `field(default=[])`) — always use `field(default_factory=list)`. (Source: https://peps.python.org/pep-0557/.)

## Purpose

These rules keep Python functions small, focused, testable, and readable:

- **Single responsibility** ensures each function can be understood, tested, and replaced in isolation.
- **Early returns** eliminate the arrow anti-pattern and reduce cognitive load when reading control flow.
- **Pure functions** are trivially unit-testable without mocks and safe to call in any order.
- **Side-effect isolation** makes the places that touch the outside world easy to find and stub out in tests.
- **Dataclasses** replace ad-hoc dicts and tuples with typed, self-documenting structures.
- **Keyword-only args** eliminate a whole class of silent argument-order bugs and make call sites self-documenting.
