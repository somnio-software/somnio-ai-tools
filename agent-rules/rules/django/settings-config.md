...
- Committing `SECRET_KEY` or database passwords directly in `settings.py` or `.env` files that are tracked by git.
- Using a single settings file for all environments with inline `if`-branch logic — makes diffs noisy and secrets easy to leak.
- Leaving `DEBUG=True` in a production-oriented settings file, exposing full tracebacks and disabling security checks.
- Wildcard or empty `ALLOWED_HOSTS` in production, bypassing Django's host-header guard.
- Starting the app with `runserver` in production — it is a single-threaded development server with no static-file serving and no hardening.
- Forgetting `STATIC_ROOT` so `collectstatic` has nowhere to write, or missing `STATICFILES_STORAGE` for fingerprinted filenames.
- Not rotating `SECRET_KEY` when it is suspected of being compromised, and not using `SECRET_KEY_FALLBACKS` to keep existing sessions valid during the rotation window.
- Setting `CONN_MAX_AGE` without `CONN_HEALTH_CHECKS`, causing silent stale-connection errors under pgBouncer or long-idle workers.

---

## Purpose

Settings files are the single point of truth for how Django is configured across environments. Misconfiguration here — leaked secrets, open `ALLOWED_HOSTS`, or `DEBUG=True` — translates directly into security vulnerabilities or outages. These rules keep environment-specific values out of source control, enforce a split-file layout that scales across teams, and gate every deploy on `check --deploy` to catch regressions automatically.
