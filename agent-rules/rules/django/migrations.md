...
- Importing a live model class directly inside a `RunPython` callable instead of calling `apps.get_model(...)`. This is the single most common data-migration bug: the callable runs correctly at authoring time but silently operates on a mismatched schema once further model changes are applied.
- Omitting `reverse_code` from `RunPython`. This is easily overlooked when writing one-shot backfills; the absence is invisible until a rollback is attempted in production under pressure.
- Combining a schema change (e.g. `AddField`) and a data backfill in the same migration. The correct pattern is two migrations: one that adds the column, one (data migration) that populates it.
- Undeclared cross-app dependencies. If migration `invoicing.0012` relies on a field added by `accounts.0008`, but `accounts.0008` is absent from `dependencies`, the migration order is non-deterministic across fresh installs and CI runs.
- Not committing migrations. Leaving migrations as local-only changes means teammates hit `InconsistentMigrationHistory` errors and CI fails on the database-setup step.
- Using vague auto-generated migration names. Reviewing `git log` or `showmigrations` output becomes difficult when every name is a timestamp.
- Running ad-hoc `ALTER TABLE` or `INSERT` statements directly on the database. Even a "quick fix" applied in production will desync the migration graph and may conflict with the next deploy.

## Purpose

Enforce safe, reviewable, and reversible schema evolution in Django projects. Migrations are executable history of the database schema; errors here are often irreversible in production and affect every team member and environment. These rules prevent the most costly migration hazards — irreversible data migrations, historical-model bypass, out-of-band schema drift — while keeping the migration graph deterministic and auditable.
