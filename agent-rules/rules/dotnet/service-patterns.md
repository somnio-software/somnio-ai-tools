...
The service (application) layer holds business logic and orchestrates repositories, domain rules, and external calls. Getting this layer right:
- Keeps controllers as thin adapters between HTTP and business logic
- Makes business rules unit-testable without a running web host
- Avoids captive dependency bugs caused by mismatched DI lifetimes
- Keeps orchestration methods readable by splitting them into named steps
- Prevents runaway concurrency when processing batches of work
...
public class OrdersController(AppDbContext dbContext) : ControllerBase
{
    ...
    {
        // Business logic, validation, and persistence all inline in the controller
        ...
        var order = new Order { CustomerId = request.CustomerId, Status = "Pending" };
        dbContext.Orders.Add(order);
        await dbContext.SaveChangesAsync();

        return Ok(order); // Untestable without a full HTTP + DB round trip
    }
}
```
...
{
    // 80+ lines mixing validation, mutation, and notification with no named steps
    var customer = await customerRepository.GetByIdAsync(request.CustomerId, default);
    if (customer == null) throw new Exception("not found");
    if (!customer.IsActive) throw new Exception("inactive");

    foreach (var item in request.Items)
    {
        var available = await inventoryService.GetAvailableStockAsync(item.ProductId, default);
        if (available < item.Quantity) throw new Exception("no stock");
    }
    ...
    await orderRepository.AddAsync(order, default);
    foreach (var item in request.Items)
    {
        await inventoryService.ReserveStockAsync(item.ProductId, item.Quantity, default);
    }
    await dbContext.SaveChangesAsync();

    await notificationService.SendOrderConfirmationAsync(customer.Email, order.Id, default);

    return order;
    // Impossible to unit test "validate customer" in isolation; generic Exception
    // types instead of distinguishable, catchable error types
}
```
...
{
    var order = await orderRepository.GetByIdAsync(orderId, default);
    order.Cancel(); // NullReferenceException if the order doesn't exist
    await dbContext.SaveChangesAsync();
    // No check on current status — cancelling a delivered order silently "succeeds"
}
```
...
```csharp
public async Task<BatchResult> ImportOrdersAsync(IReadOnlyList<ImportOrderRequest> requests)
{
    // Fires every request concurrently with no limit — thousands of orders means
    // thousands of simultaneous DbContext operations and connection pool exhaustion
    var tasks = requests.Select(r => CreateOrderAsync(r, default));
    var orders = await Task.WhenAll(tasks);

    return new BatchResult(orders.ToList(), []);
    // Also loses per-item failure information: one failing item faults the entire batch
}
```
...
- Controllers containing validation, persistence, or orchestration logic instead of delegating to a service
- Registering a service as `AddSingleton` when it depends on scoped `DbContext` (captive dependency)
- One 100+ line method doing validation, mutation, and side effects with no named sub-steps
- Mutating an entity before confirming it exists or satisfies business rules, resulting in null-reference exceptions or silently-wrong state changes
- Throwing generic `Exception` instead of specific, catchable domain exceptions
- Cargo-culting `.ConfigureAwait(false)` into every await in application code out of outdated habit
- Introducing MediatR (or another mediator library) for a handful of endpoints where it only adds indirection
- Unbounded `Task.WhenAll` over a large collection, exhausting connections or hitting downstream rate limits
- Losing per-item error information when a batch operation fails partway through
