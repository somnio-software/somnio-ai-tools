...
globs: src/**/Dtos/*.cs, src/**/Contracts/*.cs, src/**/*Request.cs, src/**/*Response.cs, src/**/*Validator.cs
alwaysApply: false
---
...
Disciplined DTOs and validation ensure:
- The wire contract is stable and intentional, decoupled from EF Core entity changes
- Invalid requests are rejected before business logic runs, with a consistent error shape
- Sensitive fields (password hashes, tokens, internal ids) never accidentally serialize to clients
- Nullability on a DTO communicates business meaning (required vs optional), not just C# type rules
- Validation logic is testable in isolation, independent of the HTTP pipeline
...
- Returning an EF Core entity (or a DTO that mirrors it 1:1) directly from an endpoint
- Reusing a single DTO across create, update, and read operations
- Letting a client set server-controlled fields (`Id`, `CreatedAt`, `Status`) through a request DTO
- Hand-rolling `if (string.IsNullOrEmpty(...))` validation that Data Annotations already provide
- Putting conditional or cross-field validation logic inside a service method instead of a validator
- Serializing password hashes, security stamps, or tokens because they were never removed from the response type
- Making every property nullable "to be safe," pushing null-checks into every consumer
- Using reflection-based or JSON round-trip "mapping" instead of an explicit method or a mapping library
- Skipping validation entirely on minimal API endpoints because there's no `[ApiController]` to auto-validate `ModelState`
