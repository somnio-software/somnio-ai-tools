...
- Opening DB sessions, HTTP clients, or other resources inline inside a route handler instead of via a `yield` dependency: the resource is never reliably closed on errors, leading to connection leaks. ([deps-yield](https://fastapi.tiangolo.com/tutorial/dependencies/dependencies-with-yield/))
- Manually caching or re-running a dependency that FastAPI already caches per request — this duplicates work and can produce stale or inconsistent state within the same request lifecycle. ([zk](https://github.com/zhanymkanov/fastapi-best-practices))
- Placing business logic or database queries directly in a route handler that could instead be extracted into a `Depends` — this prevents reuse and makes testing harder. ([zk](https://github.com/zhanymkanov/fastapi-best-practices))

## Purpose

FastAPI's `Depends` system is the primary mechanism for achieving reuse, testability, and correct resource lifecycle management. Extracting shared concerns into dependencies keeps handlers thin, enables `dependency_overrides` in tests (see `fastapi/data-and-config.md`), and ensures resources such as DB sessions are always acquired and released in a safe, predictable order. The `yield` + `try/finally` pattern is the framework-recommended way to implement setup/teardown; violating the re-raise discipline is the single most common source of silent failures in FastAPI applications.
