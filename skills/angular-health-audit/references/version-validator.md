# Angular Version Validator

> Verify nvm setup, validate Node.js version alignment, and confirm all project dependencies are installed correctly for an Angular workspace.
...
3. **Dependency Installation Verification**:
   ...
   - Check for peer dependency warnings (common across `@angular/*` majors)
   - If apps/ or libs/ exist, verify each has `node_modules`
   - Confirm the Angular CLI resolves locally: `npx ng version`

4. **Build Verification** (optional, non-blocking):
   - If a `build` script exists (`ng build`), attempt:
...
5. **Type Checking** (optional, non-blocking):
   - Angular's build already type-checks via ngc. Optionally run
     `npx tsc --noEmit > /dev/null 2>&1` for a fast TS-only check
