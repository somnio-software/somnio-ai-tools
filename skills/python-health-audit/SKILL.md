...
- At-a-Glance Scorecard with all 9 section scores, plus the `Test Coverage`
  summary line and the one-sentence Overall Score interpretation
- All 9 detailed sections (Tech Stack, Architecture, API/Interface Design,
  Data Layer, Testing, Code Quality, Documentation &
  Operations, CI/CD, AI Harness & Adoption) — Testing carries the verbatim
  `Code Coverage:` / `Coverage Breakdown:` fields from Step 0
- Additional Metrics (no coverage percentages here — coverage lives only in
  the scorecard summary line and the Testing section fields)
- Risks & Opportunities (5-8 bullets)
- Recommendations (6-10 prioritized actions)
- Appendix: Evidence Index
- Appendix: Scoring Methodology (weights table; read-only summary — the
  authoritative weights live in `references/report-generator.md`)
