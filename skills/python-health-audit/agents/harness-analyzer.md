---
name: python-health-audit-harness-analyzer
description: |
...
7. **Save Output**: Write the analysis artifact to `reports/.artifacts/python-health-audit/step_09_harness_analysis.md`.
...
- The reference gate command for this stack is `ruff check && pytest`. Treat it as a reference, not an exact-match requirement: a project wrapper (`./scripts/check.sh`, a Makefile target, an npm script) that reaches the same lint/analyze/test checks is equivalent and scores full marks. Never report "the command does not match `ruff check && pytest`" as a finding.
...
Save your complete analysis to `reports/.artifacts/python-health-audit/step_09_harness_analysis.md`.

Create the directory first: `mkdir -p "$(dirname reports/.artifacts/python-health-audit/step_09_harness_analysis.md)"`
