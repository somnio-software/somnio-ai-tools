...
|---|---|
| Expected gate command (the project's lint/analyze/test one-liner) | `ruff check && pytest` |
| Expected rule `paths:` glob (the stack's source tree) | `**/*.py` |
...
- A rule glob that is narrower or differently-shaped but still targets
  ...
  or is absent.
- Never report "the gate command does not match `ruff check && pytest`" as
