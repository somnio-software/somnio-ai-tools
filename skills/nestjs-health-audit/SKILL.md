...
Steps 1-9 can be partially parallelized using the Agent tool to launch
...
**Wave 2 (Parallel)**: Steps 3 + 4 + 5 + 9 — CI/CD + Testing + Code Quality + AI Harness & Adoption
  ...
  harness analysis depends on no prior artifact, so it joins this wave.
...
100-point rubric, judging quality, not just presence.
...
prioritised action list for the "AI Harness & Adoption" report
section.
...
- At-a-Glance Scorecard with all 9 section scores, plus the
  `> Test Coverage:` line, the scoring legend, and the Overall Score
  interpretation sentence
- All 9 detailed sections (Tech Stack, Architecture, API Design,
  Data Layer, Testing, Code Quality, Documentation &
  Operations, CI/CD, AI Harness & Adoption) — Testing carries the
  canonical Code Coverage / Coverage Breakdown fields
- Additional Metrics (no coverage percentages — those live only in
  the scorecard line and the Testing section)
- Risks & Opportunities (5-8 bullets)
- Recommendations (6-10 prioritized actions)
- Appendix: Evidence Index
- Appendix: Scoring Methodology (unnumbered; the only place weights
  appear)
...
- Reads all eleven artifacts produced by Waves 0–4.
- Computes 9 weighted section scores and overall score per the weights defined in `references/report-generator.md` — that file is the single source of truth; do not restate the numbers here.
...
Include this block at the very end of the report, as an unnumbered
Markdown table (it follows the "Appendix: Scoring Methodology" block):
