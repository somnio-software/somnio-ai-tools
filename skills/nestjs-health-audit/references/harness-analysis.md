...
|---|---|
| Expected gate command (the project's lint/analyze/test one-liner) | `npm run lint && npm test` |
| Expected rule `paths:` glob (the stack's source tree) | `src/**/*.ts` |
...
- A rule glob that is narrower or differently-shaped but still targets
  ...
  or is absent.
- Never report "the gate command does not match `npm run lint && npm test`" as
...
- Via in-session dispatch: the dispatching orchestrator names the path;
  use it verbatim. See "Subagent Dispatch (in-session)" in SKILL.md.
