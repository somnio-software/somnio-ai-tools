...
7. **Save Output**: Write the analysis artifact to `reports/.artifacts/angularjs-health-audit/step_08_harness_analysis.md`.
...
- The reference gate command for this stack is `grunt test` (JSHint + Karma; equivalently `gulp test` or `npm test`). Treat it as a reference, not an exact-match requirement: a project wrapper (`./scripts/check.sh`, a Grunt/gulp task, an npm script) that reaches the same lint/analyze/test checks is equivalent and scores full marks. Never report "the command does not match `grunt test`" as a finding.
...
Save your complete analysis to `reports/.artifacts/angularjs-health-audit/step_08_harness_analysis.md`.

Create the directory first: `mkdir -p "$(dirname reports/.artifacts/angularjs-health-audit/step_08_harness_analysis.md)"`
